// Public Golf locator only. Writes candidates, never the published catalog.
import { readFile, writeFile, rename } from 'node:fs/promises';
const baseline = JSON.parse(await readFile(new URL('./data/golf-round2-reviewed.json', import.meta.url), 'utf8'));
...
const branches = [], skipped = [], store = 'גולף';
for (const row of rows) {
  ...
  branches.push({ ...row, id: `golf-${row.id}`, store, sourceUrl, coordinateSourceUrl: sourceUrl, checkedAt, locationAccuracy: 'address-or-venue' });
}
...
const output = { store, branches, source: {
  store, aliases: ['גולף', 'GOLF', 'Golf'], sourceUrl, checkedAt,
  examined: rows.length, included: branches.length, coverage: 'partial',
  note: 'מיקומים מפורשים לכל סניף מתוך data-latitude/data-longitude ברשימת הסניפים הרשמית של גולף; לא מרכז תצוגת מפה ולא גיאוקוד משוער. הושמטו עודפים עד בירור זכאות, כפילויות וסתירות בין המיקום לעיר או למתחם. הרשימה אינה כוללת GOLF KIDS ואינה מוכיחה זכאות לשובר.',
  skipped,
} };
const target = new URL('../candidates/golf-round2.json', import.meta.url);
const temp = new URL('../candidates/golf-round2.json.tmp', import.meta.url);
