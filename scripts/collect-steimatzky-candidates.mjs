// Official Steimatzky locator -> candidates only. No shared catalog mutation.
import { readFile, writeFile, rename } from 'node:fs/promises';
const baseline = JSON.parse(await readFile(new URL('./data/steimatzky-round7-reviewed.json', import.meta.url), 'utf8'));
const sourceUrl = baseline.sourceUrl;
const response = await fetch(sourceUrl, { signal: AbortSignal.timeout(30000) });
if (!response.ok || new URL(response.url).hostname !== 'www.steimatzky.co.il') throw Error('Official Steimatzky locator unavailable');
...
for (const row of rows) {
  ...
  branches.push({ id: `steimatzky-${row.id}`, store, name: row.name, city: row.city, address: row.address,
...
const output = { store, branches, source: {
  store, aliases: ['סטימצקי', 'STEIMATZKY', 'Steimatzky'], sourceUrl, checkedAt,
  examined: rows.length, included: branches.length, coverage: 'partial',
  note: 'יעדי Waze מפורשים מרשימת סטימצקי הרשמית, לאחר התאמת העיר והמתחם או הכתובת. יעדים חשודים, כפילויות אפשריות בנמלי תעופה, משרדים, סיטונות ומתחמים הדורשים בירור נשארו לבדיקה. אין ברשומה הוכחת זכאות לשובר.',
  skipped,
} };
const target = new URL('../candidates/steimatzky-round7.json', import.meta.url);
const temp = new URL('../candidates/steimatzky-round7.json.tmp', import.meta.url);
