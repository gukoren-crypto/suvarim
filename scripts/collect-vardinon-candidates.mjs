// Official Vardinon locator -> candidates only. No shared catalog mutation.
import { readFile, writeFile, rename } from 'node:fs/promises';
const baseline = JSON.parse(await readFile(new URL('./data/vardinon-round4-reviewed.json', import.meta.url), 'utf8'));
const sourceUrl = baseline.sourceUrl;
const response = await fetch(sourceUrl, { signal: AbortSignal.timeout(30000) });
if (!response.ok || new URL(response.url).hostname !== 'www.vardinon.co.il') throw Error('Official Vardinon locator unavailable');
...
for (const row of rows) {
  ...
  branches.push({ id: `vardinon-${row.id}`, store, name: row.name, city: row.city, address: row.address,
...
const output = { store, branches, source: {
  store, aliases: ['ורדינון', 'VARDINON', 'Vardinon'], sourceUrl, checkedAt,
  examined: rows.length, included: branches.length, coverage: 'partial',
  note: 'יעדי Waze מפורשים מרשימת ורדינון הרשמית המסוננת לפי מותג, לאחר התאמת העיר והמתחם. סניפים משולבים המוזכרים ברשימה נכללו כורדינון בלבד; לא נוספו רשומות נעמן. קואורדינטות משותפות חשודות, עודפים, ירידים וכתובות או מתחמים שדורשים בירור נשארו לבדיקה. אין ברשומה הוכחת זכאות לשובר.',
  skipped,
} };
const target = new URL('../candidates/vardinon-round4.json', import.meta.url);
const temp = new URL('../candidates/vardinon-round4.json.tmp', import.meta.url);
